...
1. Query(Q)：$\mathbf{Q}=xW_q$
2. Key(K)：$\mathbf{K}=xW_k$
3. Value(V)：$\mathrbf{V}=xW_v$

这里的$W_q，W_k，W_v$是三个独立的、需要通过模型训练学习的权重矩阵。
</br>**为什么需要三个不同的矩阵？**
...
#### 第一步：切换语言——用复数描述世界
 - **二维向量→复数**：$\mathbf{q}=\begin{bmatrix}q_1\\q_2\end{bmatrix}$不再用两个数字表示，而是用一个复数。\
