...
**注：而在 RoPE 中，我们不对输入 Embedding 做加法，而是对 Attention 里的 Query (q) 和 Key (k) 向量进行旋转**。
...
**注意**：
- 这里要解释一下上面的 $q$ 和 $k$ 是经过原始的 $Q,K$ 得到的，就是都已经经过 $W_q,W_k$ 计算之后的权重，所以自带信息。
- RoPE没有改变原始 $Q,K$ 的维度，输入输出均是 $[\text{Batch},\text{Heads},\text{SeqLen},\text{HeadDim}]$ ，形状没有变化，数值完全变了，因为最后计算Attention的是 $Q=Q+Position$ 。
- 这里的 $\theta$ 是按照一个预设的**几何级数**生成的，对于维度索引 $i\varepsilon[0,d/2)$ , $\theta_i$ 的计算公式通常是： $\theta_i=10000^{-2i/d}$
   - 10000：这是一个 基数（base），是目前扩展长上下文时最常调整的超参数（有些模型改为 500,000 甚至更大）。
   - $i$ ：表示是第几组（第几个子空间）。
   - $d$ ：总维度（head_dim）。

- 其中的 $q^{'},k^{'}$ 均是在单个Token内部做的，因为2个维度才能组成一个平面，因为一个数字（比如在数轴上的一个点），是无法进行“旋转”操作的，只能把它变大或变小(缩放)，要**实现旋转**必须有一个平面，必须有两个坐标轴 $X$ 轴和 $Y$ 轴，在RoPE中是利用复数去实现旋转的。这也是为什么选择的时钟数量是 d/2 ，因为是在单个Token内做的，所有就是Head_dim决定的数量。
#### 2. 开始漫长的计算
...
在实际代码实现中，为了提升计算效率和数值稳定性，通常会预先计算频率的倒数: $\mathrm{invfreq}_i=\text{base}^{\frac{2i}{d}}$

这样，在计算旋转角度时，就可以将乘法 $m\theta_i$ 替换为等价的除法： $\frac{m}{\mathrm{invfreq}_i}$

由于 invfreq 可以在模型初始化时一次性计算完成并缓存，这避免了在每次前向传播中重复进行幂运算，从而优化了性能。需要注意的是，这是一种与原始公式在数学上等价的实现技巧，其核心思想并未改变。
...
**Answer**: 不会。这是一个常见误解。分组旋转是一种信息重组而非信息丢失：
 ...
 ## 十六、为什么 RoPE 比绝对位置（Absolute PE）好？
 统的绝对位置编码（如 Sinusoidal 或 Learnable PE）有两大硬伤，RoPE 完美解决了它们：

 ### 理由一：天然的相对位置感知

- **绝对位置的问题**：模型很难直接学到“单词 A 和单词 B 距离为 5”这个概念。它学到的是“单词 A 在第 100 位，单词 B 在第 105 位”。如果下次 A 在第 200 位，B 在 205 位，模型可能就不认识这种关系了（不具备平移不变性）。
- **RoPE 的优势**：通过旋转，Attention 分数只和 $(m-n)$ 有关。无论这两个词出现在句子的开头还是结尾，只要它们距离一样，它们的相互作用模式就是一样的。这极大地降低了模型的学习难度。

### 理由二：长度外推性 (Length Extrapolation) —— 最关键点
- **绝对位置的问题**：训练时你只练过长度为 2048 的文本。测试时突然来了个 4096 长度的文本。对于第 3000 个位置，绝对位置编码是未定义或者未训练过的，模型直接崩掉。
- **RoPE 的优势**：因为它是基于旋转角度的。虽然训练时没见过这么大的旋转角度，但数学性质依然保持。
   - 注：虽然原始 RoPE 直接外推效果也有限，但它为NTK-Aware Scaled RoPE、YaRN 等长文本扩展技术奠定了基础。通过简单的插值（把大角度压缩回训练过的范围内），RoPE 模型可以轻松做到“训练 4k，推理 100k”。这是绝对位置编码做不到的。

### 理由三：乘性耦合 vs 加性耦合
- **绝对位置**： $x+p$ 。位置信息和语义信息是“混合”在一起的。
- **RoPE**： $x\cdot R$ 。位置信息是通过“旋转”注入的，这是一种乘性操作。在几何上，乘性操作更能保留向量的模长（语义强度），只改变方向（位置关系），这在 Attention 机制（主要靠方向计算相似度）中更加优雅。

**一句话总结：RoPE 用“旋转的角度差”来表示“距离”，让大模型具备了处理超长上下文和相对位置关系的数学直觉。**